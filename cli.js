    ...
    for (let i = __analyse_period; i < __feed_size; i++) {
        if (__feed_analyse[i].symbol.match(/U/) && __feed_analyse[i - 1].symbol.match(/U/)) {
            if (__feed_analyse[i].RSI >= 16 && __feed_analyse[i].RSI <= 20) {
                console.log(' [BUY Signal] ' + __feed_analyse[i].time + ' - detected on the sequance ' + __feed_analyse[i].position + '  RSI = ' + __feed_analyse[i].RSI + ' - O: ' + __feed_analyse[i].open + '  C: ' + __feed_analyse[i].close + '  H: ' + __feed_analyse[i].high + '  L: ' + __feed_analyse[i].low);
            }
        }

        if (__feed_analyse[i].symbol.match(/D/) && __feed_analyse[i - 1].symbol.match(/D/)) {
            if (__feed_analyse[i].RSI >= 70) {
                console.log(' [Sell Signal] ' + __feed_analyse[i].time + ' - detected on the sequance ' + __feed_analyse[i].position + '  RSI = ' + __feed_analyse[i].RSI + ' - O: ' + __feed_analyse[i].open + '  C: ' + __feed_analyse[i].close + '  H: ' + __feed_analyse[i].high + '  L: ' + __feed_analyse[i].low);
            }
        }
    }
}
